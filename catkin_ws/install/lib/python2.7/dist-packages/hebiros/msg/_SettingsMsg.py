...
"""
  ...
  def deserialize(self, str):
    ...
    try:
      ...
      self.save_current_settings = list(map(bool, self.save_current_settings))
      start = end
      end += 4
      (length,) = _struct_I.unpack(str[start:end])
      pattern = '<%sb'%length
      ...
      self.position_gains.d_on_error = list(map(bool, self.position_gains.d_on_error))
      start = end
      end += 4
      (length,) = _struct_I.unpack(str[start:end])
      ...
      self.velocity_gains.d_on_error = list(map(bool, self.velocity_gains.d_on_error))
      start = end
      end += 4
      (length,) = _struct_I.unpack(str[start:end])
      ...
      self.effort_gains.d_on_error = list(map(bool, self.effort_gains.d_on_error))
      return self
